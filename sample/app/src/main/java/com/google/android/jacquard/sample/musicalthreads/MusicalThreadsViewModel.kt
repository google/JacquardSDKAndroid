...
class MusicalThreadsViewModel(
  ...
  fun init() {
    connectedJacquardTagSignal =
      connectivityManager.getConnectedJacquardTag(preferences.currentTag.address())
  }
  ...
  fun startTouchStream(): Signal<List<Int>> {
    return Signal.create { signal ->
      val subscription = getTouchData().tap { lines -> threadsPlayer.play(lines) }.forward(signal)
