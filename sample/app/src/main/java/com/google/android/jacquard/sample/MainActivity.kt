...
class MainActivity : AppCompatActivity(), ShakeDetector.ShakeListener, ApplyFirmwareListener {
    ...
    private val shakeDetector by lazy { ShakeDetector((baseContext.getSystemService(Context.SENSOR_SERVICE) as SensorManager)) }
    ...
    override fun onActivityResult(requestCode: Int, resultCode: Int, data: Intent?) {
        ...
        activityResults.next(ActivityResult(requestCode, resultCode, data))
    }
    ...
    private fun showSnackbar(message: String) {
        Util.showSnackBar(findViewById(android.R.id.content), message, Util.DURATION_4_SECONDS)
    }
            ...
            when (state.type) {
                ...
                TRANSFERRED -> {
                    if (viewModel.isAutoUpdate) {
                        ...
                        viewModel.removeFlagDfuInProgress()
                        if (!viewModel.isFirmwareUpdateFragment()) {
                            // On FirmwareUpdateFragment do not show snackbar
                            showSnackbar(
                                getString(R.string.continue_execute),
                                getString(R.string.almost_ready_update_dialog_positive_btn),
                                Util.DURATION_4_SECONDS
                            ) { viewModel.navigateToFirmwareUpdate() }
                        }
                    }
                }
                EXECUTING -> {
