...
class ScanFragment : Fragment(), ScanAdapter.ItemClickListener {
  ...
  override fun onCreate(savedInstanceState: Bundle?) {
    ...
    subscriptions.add(viewModel.stateSignal.onNext { state ->
      onState(state)
    })
  }
  ...
  private fun onState(state: State) {
    when (state.getType()) {
      ...
      State.Type.CONNECTED -> onConnectionState()
      State.Type.ERROR -> {
        Util.showSnackBar(view, state.error())
