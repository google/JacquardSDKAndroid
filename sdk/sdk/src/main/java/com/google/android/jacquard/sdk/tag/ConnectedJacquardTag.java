 ...
 *
 * <p>A connected tag is obtained by calling {@link JacquardManager#connect(AdvertisedJacquardTag)}.
 */
public interface ConnectedJacquardTag extends JacquardTag {
  ...
  Signal<List<Module>> getRemoteModules();
   ...
   */
  <Res, Request extends ProtoCommandRequest<Res>> Signal<Res> enqueue(Request request, int retries, long timeout);
   ...
   * @return a Signal emitting a single response
   */
  <Res, Request extends ProtoCommandRequest<Res>> Signal<Res> enqueue(Request request, int retries);
   ...
   * @return a Signal emitting a single response
   */
  <Res, Request extends ProtoCommandRequest<Res>> Signal<Res> enqueue(Request request);

  /** Send non-fragmented data to the ujt over data/raw characteristics. */
  Signal<Boolean> sendData(byte[] packet);

  /** Returns a signal to receive ack packets sent by ujt. */
  Signal<Byte> getAckSignal();
}
