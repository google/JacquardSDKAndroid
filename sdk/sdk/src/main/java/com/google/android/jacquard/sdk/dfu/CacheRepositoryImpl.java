...
final class CacheRepositoryImpl implements CacheRepository{
   ...
   */
  @Override
  public Signal<String> cacheDescriptor(DFUInfo dfuInfo, FileDescriptor descriptor) {
   ...
   */
  private boolean removeCacheDescription(DFUInfo dfuInfo) {
  ...
  public Optional<FileDescriptor> getDescriptor(DFUInfo dfuInfo) {
    try {
      Pair<InputStream, Long> pair =
          DfuUtil.getFileInputStream(/* path= */ dfuInfo.downloadFilePath());
      if (pair.first == null) {
   ...
   * @param currentVersion <code>String</code> Current component version.
   */
  @Override
  public void cacheUpdateInformation(DFUInfo dfuInfo,
      String vid, String pid, String mid, String currentVersion,
      long timeInMilliseconds) {
    sharedPreferences.edit()
        .putString(imageInfoKey(vid, pid, mid, currentVersion),
            new Gson().toJson(dfuInfo))
        .apply();
    sharedPreferences.edit()
        .putLong(imageInfoKey(vid, pid, mid, currentVersion) + "_time", timeInMilliseconds)
        .apply();
  }
  ...
  public Optional<DFUInfo> getUpdateInformation(String vid,
    ...
    String jsonDfuInfo = sharedPreferences
        .getString(imageInfoKey(vid, pid, mid, currentVersion), /* defValue= */"");
    ...
    if (isCachePeriodInvalidated(sharedPreferences
        .getLong(imageInfoKey(vid, pid, mid, currentVersion) + "_time", 0L))) {
      return Optional.absent();
    }
