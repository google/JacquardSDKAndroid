...
class DFUChecker {
    ...
    checkUpdate(checkUpdateParamsList.get(indexPos), forceUpdate).tapError(signal::error)
        .onNext(dfuInfo -> {
          if (!dfuInfo
              .version()
              .toZeroString()
              .equals(checkUpdateParamsList.get(indexPos).componentVersion())) {
  ...
  Signal<DFUInfo> checkUpdate(CheckUpdateParams checkUpdateParams, boolean forceUpdate) {
    ...
    Optional<DFUInfo> optionalDfu = cacheRepository
        .getUpdateInformation(checkUpdateParams.vendorId(),
            ...
            checkUpdateParams.componentVersion());
      ...
      asSignal(call)
          ...
          .onNext(dfu -> {
            ...
            downloadFirmware(dfu)
                .observeOn(Executors.mainThreadExecutor())
                .map(downloadFilePath -> {
                  DFUInfo updateDfu = DFUInfo.create(
                      dfu.version().toZeroString(),
                      dfu.dfuStatus().name(),
                      dfu.downloadUrl(),
                      dfu.vendorId(),
                      dfu.productId(),
                      dfu.moduleId(),
                      downloadFilePath);
                  cacheRepository.cacheUpdateInformation(updateDfu,
                      checkUpdateParams.vendorId(),
                      checkUpdateParams.productId(),
                      checkUpdateParams.moduleId(),
                      checkUpdateParams.componentVersion(),
                      Calendar.getInstance().getTimeInMillis());
                  signal.next(updateDfu);
                  signal.complete();
                  return true;
  ...
  private Signal<String> downloadFirmware(DFUInfo dfu) {
