...
public class JacquardManagerImplTest {
  ...
  public void setUp() {
    ...
    jacquardManager =
        new JacquardManagerImpl(
            context, bleAdapter, new FakeLocalRemoteFunction(context.getResources()));
  }
  ...
  public void getConnectionStateSignal_withError() {
    ...
    // Assert
    assertThat(throwableReference.get()).isInstanceOf(JacquardError.class);
  }

  @Test
  public void getConnectionStateSignal_withSuccess() {
    // Assign
    AtomicReference<ConnectionState> connectionStateReference = new AtomicReference<>();
    BluetoothAdapter.getDefaultAdapter().enable();
    jacquardManager.connect(context, FAKE_ADDRESS, consumer).consume();
