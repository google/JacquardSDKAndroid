...
public class FakeDfuManager implements DfuManager {
  ...
  @Override
  public Signal<List<DFUInfo>> checkFirmware(List<Component> components, String vid, String pid,
      boolean forceUpdate) {
    return Signal.just(ImmutableList.of(getTagDfuInfo()));
  }
  ...
  public Signal<FirmwareUpdateState> applyUpdates(List<DFUInfo> dfuInfos, boolean autoExecute) {
    return Signal.<FirmwareUpdateState>create(
        signal -> {
          signal.next(FirmwareUpdateState.ofTransferProgress(100));
          signal.next(FirmwareUpdateState.ofTransferred());
          if (autoExecute) {
            signal.next(FirmwareUpdateState.ofExecuting());
          }
          signal.next(FirmwareUpdateState.ofCompleted());
          return new Subscription();
        }).tap(stateSignal::next);
  }
  ...
  private List<DFUInfo> getModuleDfuInfo() {
    List<DFUInfo> infoList = new ArrayList<>();
    infoList.add(
        DFUInfo.create(
            "000017000", "MANDATORY", URI.create("http://test.com"), LM_VID, LM_PID, LM_MID));
    return infoList;
  }
}
