...
public class FakeTransportImpl extends TransportImpl {
  ...
  private final Map<String, ConfigElement> configValues = new HashMap<>();
  ...
  @Override
  public <ProtoRequest extends ProtoCommandRequest<?>> Signal<byte[]> enqueue(
      ProtoRequest req, WriteType writeType, int retries, long timeout) {
    JacquardProtocol.Request request = req.getRequest();
        ...
        case BASE:
          switch (request.getOpcode()) {
            ...
            case CONFIG_GET:
              ...
              break;
            case CONFIG_SET:
              ConfigSetRequest protoReq = request.getExtension(ConfigSetRequest.configSetRequest);
              configValues.put(getConfigKey(protoReq.getVid(), protoReq.getPid(),
                  protoReq.getConfig().getKey()), protoReq.getConfig());
  ...
  public Signal<byte[]> getNotifySignal() {
  ...
  private void sendListModuleResponse(Signal<byte[]> responseSignal) {
    ...
    responseSignal.next(response.toByteArray());
  }

  private void sendLoadModuleResponse(Signal<byte[]> responseSignal) {
  ...
  private void sendUnloadModuleResponse(Signal<byte[]> responseSignal) {
    Response response = Response.newBuilder().setId(3)
        .setComponentId(1).setStatus(Status.STATUS_OK).build();
    responseSignal.next(response.toByteArray());
  }
  ...
  private void sendLoadModuleNotification() {
    ...
    getNotifySignal().next(notification.toByteArray());
  }

  private void sendEraseAllImuSessionsResponse(Request request, Signal<byte[]> responseSignal) {
  ...
  private void sendEraseImuSessionResponse(Request request, Signal<byte[]> responseSignal) {
  ...
  private void sendGetImuSessionListResponse(Request request, Signal<byte[]> responseSignal) {
  ...
  private void sendDataCollectionStartResponse(Request request, Signal<byte[]> responseSignal) {
  ...
  private void sendDataCollectionStopResponse(Request request, Signal<byte[]> responseSignal) {
  ...
  private void sendDataCollectionStatusResponse(Request request, Signal<byte[]> responseSignal) {
  ...
  private void sendDfuStatusResponse(Signal<byte[]> signal, Response response) {
    if (shouldThrowErrorDfuStatus) {
      signal.error(new Exception());
      return;
    }
    signal.next(response.toByteArray());
  }

  private void sendDfuPrepareResponse(Signal<byte[]> signal) {
    ...
    Response rr = Response.newBuilder().setId(2).setComponentId(1).setStatus(Status.STATUS_OK)
        .build();
    signal.next(rr.toByteArray());
  }

  private void sendDfuWriteResponse(Signal<byte[]> signal, Response response) {
  ...
  private void sendConfigReadResponse(Signal<byte[]> signal) {
  ...
  private void sendConfigSetResponse(Signal<byte[]> responseSignal) {
    Response rr = Response.newBuilder().setId(1)
        .setComponentId(1).setStatus(Status.STATUS_OK).build();
    responseSignal.next(rr.toByteArray());
  }

  private void sendConfigGetResponse(Request request, Signal<byte[]> responseSignal) {
    ...
    if (element == null) {
      responseSignal.error(new Exception(Status.ERROR_APP_UNKNOWN.toString()));
    } else {
  ...
  private void sendSetTouchModeResponse(Signal<byte[]> signal) {
    Response rr = Response.newBuilder().setId(1)
        .setComponentId(1).setStatus(
            assertFailure ? Status.ERROR_UNKNOWN : Status.STATUS_OK).build();
    signal.next(rr.toByteArray());
  ...
  private void sendConfigWriteResponse(Signal<byte[]> signal) {
    sendConfigReadResponse(signal);
  }

  private void sendDeviceBatteryResponse(Signal<byte[]> signal) {
  ...
  private void sendDeviceInfoResponse(Signal<byte[]> signal) {
    if (shouldThrowError) {
      signal.error(new Exception());
      return;
    }
