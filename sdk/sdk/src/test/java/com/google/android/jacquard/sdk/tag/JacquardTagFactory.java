...
public class JacquardTagFactory {
  ...
  public static ConnectedJacquardTag createConnectedJacquardTag(int major, int minor, int point) {
    return instance.createFakeConnectedTag(
        /* isModulePresent= */ true, /* isActive= */ false, getDeviceInfo(major, minor, point));
  }
  ...
  public static ConnectedJacquardTag createConnectedJacquardTag(
      boolean isModulePresent, boolean isActive) {
    return instance.createFakeConnectedTag(isModulePresent, isActive, getDeviceInfo());
  }

  private ConnectedJacquardTag createFakeConnectedTag(
      boolean isModulePresent, boolean isActive, DeviceInfo deviceInfo) {
    if (tag == null) {
      transport = new FakeTransportImpl(peripheral, requiredCharacteristics,
          transportState);
      tag = new ConnectedJacquardTagImpl(transport, deviceInfo, new FakeDfuManager());
    }
  ...
  private static DeviceInfo getDeviceInfo(int major, int minor, int point) {
    return DeviceInfo.ofTag(
        DeviceInfoResponse.newBuilder().setGearId("").setSkuId("").setVendorId(VID)
            .setProductId(PID).setUuid(UUID).setVendor("").setRevision(0).setModel("")
            .setMlVersion("").setBootloaderMajor(0).setBootloaderMinor(0)
            .setFirmwarePoint(point).setFirmwareMinor(minor).setFirmwareMajor(major)
            .build());
  }
