...
public final class TagConnectionStateMachineTest {
  ...
  public void setup() {
    ...
    connectionStateMachine =
        new TagConnectionStateMachine(
            /* device= */ null,
            /* badFirmwareVersions= */ Collections.singletonList(Revision.create(1, 68, 0)),
            this::doConnect, /* targetUjtFirmwareVidPid = */null);
  ...
  public void connect_tagPairingStateDisconnected_emitsPairingToConnect() {
    // Arrange
    connectState.setType(Type.VALUE_RSSI);
    connectState.setPeripheral(peripheral);
    // Act
    connectionStateMachine.connect(/* isUserInitiated= */ true);
    // Assert
    assertThat(connectionState.getType())
        .isEqualTo(ConnectionState.ofPreparingToConnect().getType());
  }
  ...
  @Test
  public void connect_tagPairingStateError_emitsPreparingToConnectState() {
    // Arrange
    connectState.setType(Type.VALUE_RSSI);
    connectState.setPeripheral(peripheral);
    ...
    connectionStateMachine.connect(tagPairingStateMachine);
    // Assert
    assertThat(connectionState.getType()).isEqualTo(ConnectionState.Type.PREPARING_TO_CONNECT);
  }
  ...
  @Test
  public void connect_tagPairingStateCharacteristicsUpdated_emitsFirmwareExecutingState() {
    // Arrange
    ...
    connectionStateMachine
        ...
        .filter(
            state ->
                state.isType(ConnectionState.Type.FIRMWARE_UPDATE_INITIATED)
                    || state.isType(ConnectionState.Type.FIRMWARE_TRANSFERRING)
                    || state.isType(ConnectionState.Type.FIRMWARE_TRANSFER_COMPLETE)
                    || state.isType(ConnectionState.Type.FIRMWARE_EXECUTING))
        .onNext(list::add);
    JacquardManagerInitialization.initJacquardManager();
    JacquardTagFactory.resetTag();
    ConnectedJacquardTag tag = JacquardTagFactory.createConnectedJacquardTag(1, 68, 0);
    connectState.setType(Type.VALUE_RSSI);
    connectionStateMachine.connect(/* isUserInitiated= */ true);
    protocolInitializationStateMachine.setInitializationState(
        InitializationState.ofTagInitialized(tag));
    ...
    assertThat(list.get(0).getType()).isEqualTo(ConnectionState.Type.FIRMWARE_UPDATE_INITIATED);
    assertThat(list.get(1).getType()).isEqualTo(ConnectionState.Type.FIRMWARE_TRANSFERRING);
    assertThat(list.get(2).getType()).isEqualTo(ConnectionState.Type.FIRMWARE_TRANSFER_COMPLETE);
    assertThat(list.get(3).getType()).isEqualTo(ConnectionState.Type.FIRMWARE_EXECUTING);
  }

  @Test
  public void
      initializeConnection_protocolInitializationTagInitialized_emitsPreparingToConnectState() {
    // Arrange
    onTagPairingStateServiceDiscovered();
    connectionStateMachine.connect(tagPairingStateMachine);
    onTagPairingStateCharacteristicUpdated();
  ...
  @Test
  public void initializeConnection_protocolInitializationError_emitsPreparingToConnectState() {
    // Arrange
    connectState.setType(Type.VALUE_RSSI);
    protocolInitializationStateMachine.setInitializationState(
        InitializationState.ofError(new Throwable()));
