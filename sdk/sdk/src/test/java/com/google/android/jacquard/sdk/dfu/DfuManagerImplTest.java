...
import com.google.android.jacquard.sdk.tag.FakeConnectedJacquardTagImpl;
...
public class DfuManagerImplTest {
  ...
  private static final String PERIPHERAL_IDENTIFIER = "C2:04:1C:6F:02:BA";
  ...
  public void assignCheckFirmware_actComponentsEmpty_assertError() {
    ...
    DfuManagerImpl dfuManager = createDFUManager(getTransport(20));
  ...
  public void assignCheckFirmware_actDisconnectedTag_assertError() {
    ...
    // Assign
    final Class expectedExceptionClass = JacquardError.class;
    ...
    // Assert
    assertThat(atomicError.get()).isInstanceOf(expectedExceptionClass);
    assertThat(((JacquardError) atomicError.get()).getType())
        .isEqualTo(JacquardError.Type.BLUETOOTH_CONNECTION_ERROR);
  }
  ...
  public void assignCheckFirmware_actCheckUpdate_assertDataCorrect()
    ...
    DfuManagerImpl dfuManager = createDFUManager(
        getTransport(20), dfuChecker, new FirmwareUpdateStateMachine(), Signal.empty());
    ...
    for (DFUInfo dfu : atomicReference.get()) {
      Optional<DFUInfo> optional = cacheRepository.getUpdateInformation(component.vendor().id(),
          component.product().id(), /* mid= */"", component.version().toZeroString());
  ...
  public void assignCheckModuleUpdate_actCheckUpdate_assertDataCorrect()
    ...
    // Act
    dfuManager.checkModuleUpdate(Lists.newArrayList(module), true).forward(signal);
    shadowOf(getMainLooper()).idleFor(Duration.ofSeconds(5));

    // Assert
    assertThat(atomicReference.get()).isNotNull();
    Optional<DFUInfo> optional = cacheRepository.getUpdateInformation(module.vendorId(),
        module.productId(), module.moduleId(), module.version().toZeroString());
  ...
  public void assignApplyUpdatesAutoExecuteFalse_actIllegalState_assertError()
    ...
    DfuManagerImpl dfuManager = createDFUManager(getTransport(20), dfuChecker, fwStateMachine, signal);
    AtomicReference<Throwable> atomicError = new AtomicReference<>();
    ...
    dfuManager.applyUpdates(Collections.emptyList(), false).consume();
  ...
  public void assignApplyUpdatesAutoExecuteFalse_actApplyFirmware_assertStateCorrect()
    ...
    dfuInfo = getDfuWithDownloadFilePath(dfuInfo,file.getPath());
    ...
    DFUChecker dfuChecker = new DummyDFUChecker<>(remoteDfuInfo, /* success= */true)
        .createDownloadManager(file);
    FirmwareUpdateStateMachine fwStateMachine = new FirmwareUpdateStateMachine();
    DfuManagerImpl dfuManager = createDFUManager(fakeTransport, dfuChecker, fwStateMachine, signal);
    ...
    // Act
    fwStateMachine.getState().next(FirmwareUpdateState.ofIdle());
    dfuManager.applyUpdates(Collections.singletonList(dfuInfo), false).consume();
    countDownLatch.await(5, TimeUnit.SECONDS);
  ...
  public void assignApplyUpdatesAutoExecuteFalse_actApplyFirmware_stop()
    ...
    assertThat(list.get(1).getType()).isEqualTo(FirmwareUpdateState.Type.PREPARING_TO_TRANSFER);
    assertThat(list.get(2).getType()).isEqualTo(FirmwareUpdateState.Type.TRANSFER_PROGRESS);
    assertThat(list.get(3).getType()).isEqualTo(FirmwareUpdateState.Type.STOPPED);
  }
  ...
  public void assignApplyUpdatesAutoExecuteTrue_actApplyFirmware_assertStateCorrect()
    ...
    // Act
    fwStateMachine.getState().next(FirmwareUpdateState.ofIdle());
    dfuManager.applyUpdates(Collections.singletonList(dfuInfo), true).consume();
    countDownLatch.await(5, TimeUnit.SECONDS);
    ...
    assertThat(list.get(7).getType()).isEqualTo(FirmwareUpdateState.Type.IDLE);
  }
  ...
  public void assignApplyModuleUpdates_actApplyFirmware_assertStateCorrect()
    ...
    // Act
    fwStateMachine.getState().next(FirmwareUpdateState.ofIdle());
    dfuManager.applyModuleUpdate(Lists.newArrayList(dfuInfo)).consume();
    countDownLatch.await(5, TimeUnit.SECONDS);
    ...
    assertThat(list.get(6).getType()).isEqualTo(FirmwareUpdateState.Type.IDLE);
  }
  ...
  private DfuManagerImpl createDFUManager(
      FakeTransportImpl fakeTransport,
      DFUChecker dfuChecker,
      FirmwareUpdateStateMachine fwUpdateStateMachine,
      Signal<FirmwareUpdateState> signal) {
    FakeConnectedJacquardTagImpl fakeConnectedJacquardTag =
        ConnectedJacquardTagInitialization.createFakeConnectedJacquardTag(
            fakeTransport, getDeviceInfo());
    DfuManagerImpl dfuManager =
        new DfuManagerImpl(fakeConnectedJacquardTag, dfuChecker, fwUpdateStateMachine, signal);
    fakeConnectedJacquardTag.setDfuManager(dfuManager);
    return dfuManager;
  }

  private DfuManagerImpl createDFUManager(FakeTransportImpl fakeTransport) {
    FakeConnectedJacquardTagImpl fakeConnectedJacquardTag =
        ConnectedJacquardTagInitialization.createFakeConnectedJacquardTag(
            fakeTransport, getDeviceInfo());
    DfuManagerImpl dfuManager =
        new DfuManagerImpl(fakeConnectedJacquardTag);
    fakeConnectedJacquardTag.setDfuManager(dfuManager);
    return dfuManager;
  }

  private DFUInfo getDfuWithDownloadFilePath(DFUInfo dfuInfo, String path) {
    return DFUInfo.create(
        dfuInfo.version().toZeroString(),
        dfuInfo.dfuStatus().name(),
        dfuInfo.downloadUrl(),
        dfuInfo.vendorId(),
        dfuInfo.productId(),
        dfuInfo.moduleId(),
